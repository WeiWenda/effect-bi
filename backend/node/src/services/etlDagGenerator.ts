...
/** @deprecated 已由 auto_generate_{etl_task_info.id} 取代；保留便于迁移期清理旧文件名 */
...
export function autoGenerateDagId(etlTaskInfoId: number): string {
  return `auto_generate_${etlTaskInfoId}`;
}
...
/** `YYYY-MM-DD` → Airflow DAG start_date 分量；无效或缺失时用当前 UTC 日 */
function resolveDagStartDateParts(iso: string | undefined): { y: number; m: number; d: number } {
...
export function renderDagPython(payload: DagGeneratePayload): { dagId: string; pythonSource: string } {
  const dagId = autoGenerateDagId(payload.etlTaskInfoId);
  const opts = (payload.airflowOptions || {}) as AirflowOptions;
  const retries = typeof opts.retries === 'number' ? opts.retries : 1;
  const retryDelayMinutes = typeof opts.retryDelayMinutes === 'number' ? opts.retryDelayMinutes : 1;
  ...
  const startParts = resolveDagStartDateParts(opts.scheduleStartDate);
  const startDateLine = `    start_date=datetime(${startParts.y}, ${startParts.m}, ${startParts.d}, tzinfo=timezone.utc),`;
  ...
  lines.push('        tid = start["taskInstanceId"]');
        lines.push('        # ----- Simulated transform: random 1-3 min + 25% fail (Node PATCH complete failed, then retry) -----');
  lines.push('        # Replace with real transform (Trino / Spark); SQL reference below');
  for (const cl of sqlComments.split('\n')) {
    lines.push(`        ${cl}`);
  }
  lines.push('        # ---------------------------------------------------------------------');
  lines.push('        sleep_sec = random.randint(60, 180)');
  lines.push('        time.sleep(sleep_sec)');
        lines.push('        if random.random() < 0.25:');
        lines.push('            err = "Simulated main_transform failure (probability 0.25); DAG default_args retries apply."');
  lines.push('            fail_body = {');
  lines.push('                "status": "failed",');
  lines.push('                "endedAt": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),');
  lines.push('                "errorMessage": err,');
  lines.push('            }');
  lines.push('            _patch_json(f"/api/etl/task-runs/{tid}/complete", fail_body)');
  lines.push('            raise RuntimeError(err)');
  lines.push('        end_body = {');
