...
/**
 * 目录拖拽会传小数 sort_order（两档中点）；DB 列为 INTEGER。
 * 非整数用 Math.ceil 得到插槽（例 2.5 → 3）：先对同 parent 下 sort_order >= 槽位且非本行的记录 +1，再写入本行槽位。
 */
function etlFolderSortOrderSlot(raw: unknown): number | null {
...
function mapTaskVersion(row: Record<string, unknown>) {
  return {
    id: row.id,
    etlTaskInfoId: row.etl_task_info_id as number,
    name: row.name,
    remark: row.remark,
...
router.put('/folders/:id', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const existing = await pool.query(
      'SELECT id, parent_id, sort_order FROM etl_folders WHERE id = $1',
      [id]
    );
    ...
    const row = existing.rows[0] as { parent_id: number | null };

    const wantsReorder =
      'sortOrder' in body &&
      body.sortOrder !== undefined &&
      body.sortOrder !== null &&
      String(body.sortOrder as string | number).trim() !== '';
    const slot = wantsReorder ? etlFolderSortOrderSlot(body.sortOrder) : null;
    ...
    if ('parentId' in body) {
      ...
      values.push(body.parentId == null || body.parentId === '' ? null : body.parentId);
    }
    if (slot !== null) {
    ...
    if (slot !== null) {
      ...
      try {
        ...
        await client.query(
          ...
          [effectiveParent, id, slot]
        );
        values.push(id);
        const result = await client.query(
          `UPDATE etl_folders SET ${updates.join(', ')} WHERE id = $${p} RETURNING id, name, parent_id, sort_order, created_at, updated_at`,
          values
        );
        await client.query('COMMIT');
...
/**
 * 按 etl_task_info 主键解析逻辑任务名（与 Neo4j 表节点 `etl_task_id` 一致）。
 * GET /api/etl/task-name-by-info-id?id=
 */
router.get('/task-name-by-info-id', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!Number.isFinite(id) || id < 1) {
      res.status(400).json({ error: 'id query must be a positive integer' });
      return;
    }

    const result = await pool.query(`SELECT name FROM etl_task_info WHERE id = $1 LIMIT 1`, [id]);
    ...
    console.error('task-name-by-info-id:', error);
    res.status(500).json({ error: 'Failed to resolve task name' });
  }
});
...
/**
 * 原子更新逻辑任务名（可选改名）与产出表三元组；同步 Airflow 部署记录中的 logical_task_name。
 * PATCH body: { fromName, name, taskOutput: { catalogName, databaseName, tableName } }
 */
router.patch('/tasks/profile', async (req: Request, res: Response): Promise<void> => {
  const fromName = typeof req.body?.fromName === 'string' ? req.body.fromName.trim() : '';
  const toName = typeof req.body?.name === 'string' ? req.body.name.trim() : '';
  const taskOutput = req.body?.taskOutput;
  if (!fromName) {
  ...
  try {
    await client.query('BEGIN');
    const exists = await client.query('SELECT 1 FROM etl_task_info WHERE name = $1 LIMIT 1', [fromName]);
    ...
    if (toName !== fromName) {
      const conflict = await client.query('SELECT 1 FROM etl_task_info WHERE name = $1 LIMIT 1', [toName]);
      ...
      await client.query(
        `UPDATE etl_task_info SET name = $1, updated_at = CURRENT_TIMESTAMP WHERE name = $2`,
        [toName, fromName]
      );
      await client.query(
        `UPDATE etl_airflow_deployments SET logical_task_name = $1 WHERE logical_task_name = $2`,
        [toName, fromName]
      );
    }
    const result = await client.query(
      `UPDATE etl_task_info
       SET catalog_name = NULLIF($1, ''),
           ...
           updated_at = CURRENT_TIMESTAMP
       WHERE name = $4
       RETURNING name, catalog_name, database_name, table_name`,
      [cn, dn, tn, toName]
    );
    if (result.rows.length === 0) {
      await client.query('ROLLBACK');
      res.status(404).json({ error: 'Task not found after update' });
      return;
    }
    await client.query('COMMIT');
    const row = result.rows[0];
    ...
    try {
      ...
      /* no active txn after COMMIT or double rollback */
    }
    console.error('Error patching task profile:', error);
    res.status(500).json({ error: 'Failed to update task profile' });
  } finally {
    client.release();
  }
});
...
router.get('/tasks/versions', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await pool.query(
      `SELECT v.id, i.id AS etl_task_info_id, i.name, v.remark, v.is_published, v.sql_main,
  ...
  try {
    ...
    if (dagDir) {
      try {
        ...
        const dagFilePath = path.join(dagDir, `${dagResult.dagId}.py`);
        fs.writeFileSync(dagFilePath, dagResult.pythonSource, 'utf8');
        const absDir = path.resolve(dagDir);
        const absFile = path.resolve(dagFilePath);
        console.log(`ETL publish: DAG written — directory (resolved): ${absDir}`);
        console.log(`ETL publish: DAG written — file (resolved): ${absFile}`);
      } catch (werr) {
        await client.query('ROLLBACK');
