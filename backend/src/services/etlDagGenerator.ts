...
export function renderDagPython(payload: DagGeneratePayload): { dagId: string; pythonSource: string } {
  ...
  lines.push('        # ----- Simulated transform: random 1-3 min + 50% fail (Node PATCH complete failed, then retry) -----');
  ...
  lines.push('        if random.random() < 0.5:');
  lines.push('            err = "Simulated main_transform failure (probability 0.5); DAG default_args retries apply."');
